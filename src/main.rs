...
enum RemoveMethod {Unlink, Wipe, Wipesync}
#[derive(Parser)]
#[command(name = "shred")]
#[command(about = "Securely overwrite files to hide their contents")]
struct Args {
    /// Files to shred
    #[arg(required = true)] files: Vec<String>,
    /// Number of overwrite passes
    #[arg(short = 'n', long = "iterations", default_value = "3")] passes: u32,
    ...
    default_missing_value = "unlink", require_equals = true)] remove: Option<RemoveMethod>,
    ///  Skip confirmation prompt
    #[arg(short, long)] force: bool,
    /// Add a final pass with zeroes to hide shredding
    #[arg(short,long)] zero: bool,
}
fn overwrite_file(file: &mut std::fs::File, file_size: u64, use_random: 
    bool, quiet: bool) -> std::io::Result<()> {
    const BUFFER_SIZE: usize = 4096;
    let mut buffer = [0u8; BUFFER_SIZE];
    if use_random {rand::rng().fill_bytes(&mut buffer);}
    ...
    if remaining > 0 {file.write_all(&buffer[..remaining])?;}
    if let Some(ref pb) = progress {pb.finish_and_clear();}
    file.sync_all()?;
    Ok(())
}  
...
fn wipe_filename(path: &str, sync: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let random_name: String = (0..name_len)
        .map(|_| (b'a' + (rand::random::<u8>() % 26)) as char).collect();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Args::parse();
    for filename in &args.files {validate_file(filename, args.force)?;}
    for filename in &args.files {
        let mut file = OpenOptions::new().write(true).read(true).open(filename)?;
        let file_size = file.metadata()?.len();
        let passes = args.passes;
        for i in 1..=passes {
            if !args.quiet {println!("{}: Pass {}/{} (random)...", filename, i, passes);}
            overwrite_file(&mut file, file_size, true, !args.quiet)?;
        }
        if args.zero {
            if !args.quiet {println!("{}: Final pass (zeros)...", filename);}
            overwrite_file(&mut file, file_size, false, !args.quiet)?;
        }
        if !args.quiet {println!("Shredded '{}' ({} bytes)...", filename, file_size);}
        if let Some(method) = &args.remove {
            match method {
                RemoveMethod::Unlink => {fs::remove_file(filename)?;}
                RemoveMethod::Wipe => {wipe_filename(filename, false)?;}
                RemoveMethod::Wipesync => {wipe_filename(filename, true)?;}
            } if !args.quiet {println!("File '{}' removed.", filename);}
        }
    }
    Ok(())
}